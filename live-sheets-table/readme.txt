...
* Fixed: a column of dates sorted by the day of the month. Sorting read every value as a number, and a number is all it kept: "15.01.2026" came out as 15.01, so dates in one month tied and the order inside them was whatever it had been. Dates written day first — 15.01.2026, 15.01.26, and either with a time after it — now sort as dates. A two-digit year is read the way every spreadsheet reads one: 00 to 29 is this century, 30 to 99 the last, so a sheet sorts on the page the way it sorts in Google Sheets.
