...
<div class="wrap lstab-admin lstab-editor">
	 ...
	 *
	 * The list of columns below does the same job with checkboxes, and people
	 * kept not finding it — a checkbox called "Include" beside a heading is a
	 * long way from "I do not want that column". Here the sheet itself is the
	 * control: click a heading, click a row, and it greys out. The two stay in
	 * step, so whichever one you reach for, the other agrees.
	 */
	$lstab_sheet_rows   = isset( $source['data']['rows'] ) ? (array) $source['data']['rows'] : array();
	$lstab_sheet_heads  = isset( $source['data']['headers'] ) ? (array) $source['data']['headers'] : array();
	$lstab_hidden_rows  = isset( $source['hidden_rows'] ) ? (array) $source['hidden_rows'] : array();
	$lstab_picker_limit = 200;
	$lstab_shown_rows   = array_slice( $lstab_sheet_rows, 0, $lstab_picker_limit );
	?>

	<?php if ( $lstab_sheet_heads || $lstab_shown_rows ) : ?>
		<div class="lstab-card lstab-picker-card">
			<h2 class="lstab-card-title"><?php esc_html_e( 'Hide columns and rows', 'live-sheets-table' ); ?></h2>
			<p class="lstab-help">
				<?php esc_html_e( 'Click a heading to drop that column, or a row to drop that row. Click again to bring it back. Nothing is written to Google — your spreadsheet keeps everything, the table just stops showing it.', 'live-sheets-table' ); ?>
			</p>

			<div class="lstab-picker-scroll">
				<table class="lstab-picker" id="lstab-picker">
					<thead>
						<tr>
							<th class="lstab-picker-corner" scope="col"><span class="screen-reader-text"><?php esc_html_e( 'Row', 'live-sheets-table' ); ?></span></th>
							<?php foreach ( $lstab_sheet_heads as $lstab_i => $lstab_head ) : ?>
								<?php $lstab_col_hidden = ! empty( $lstab_columns[ $lstab_i ]['hidden'] ); ?>
								<th scope="col" class="lstab-picker-col<?php echo $lstab_col_hidden ? ' is-hidden' : ''; ?>">
									<button type="button"
										class="lstab-picker-toggle"
										data-lstab-column="<?php echo esc_attr( (string) $lstab_i ); ?>"
										aria-pressed="<?php echo $lstab_col_hidden ? 'true' : 'false'; ?>">
										<span class="lstab-picker-name">
											<?php
											echo esc_html(
												'' === trim( (string) $lstab_head )
													/* translators: %d: column number. */
													? sprintf( __( 'Column %d', 'live-sheets-table' ), (int) $lstab_i + 1 )
													: $lstab_head
											);
											?>
										</span>
										<span class="lstab-picker-mark" aria-hidden="true"></span>
									</button>
								</th>
							<?php endforeach; ?>
						</tr>
					...
					<tbody>
						<?php foreach ( $lstab_shown_rows as $lstab_r => $lstab_row ) : ?>
							<?php
							$lstab_key        = LSTAB_Hidden_Rows::key_for( $lstab_row );
							$lstab_row_hidden = LSTAB_Hidden_Rows::is_hidden( $lstab_row, $lstab_hidden_rows );
							?>
							<tr class="lstab-picker-row<?php echo $lstab_row_hidden ? ' is-hidden' : ''; ?>"
								data-lstab-key="<?php echo esc_attr( $lstab_key ); ?>">
								<th scope="row" class="lstab-picker-handle">
									<button type="button"
										class="lstab-picker-toggle"
										data-lstab-row="<?php echo esc_attr( $lstab_key ); ?>"
										aria-pressed="<?php echo $lstab_row_hidden ? 'true' : 'false'; ?>"
										<?php disabled( '' === $lstab_key ); ?>>
										<span class="lstab-picker-number"><?php echo esc_html( (string) ( $lstab_r + 1 ) ); ?></span>
										<span class="lstab-picker-mark" aria-hidden="true"></span>
									</button>
								</th>
								<?php foreach ( $lstab_sheet_heads as $lstab_i => $lstab_unused_head ) : ?>
									<?php $lstab_col_hidden = ! empty( $lstab_columns[ $lstab_i ]['hidden'] ); ?>
									<td class="lstab-picker-cell<?php echo $lstab_col_hidden ? ' is-hidden' : ''; ?>"
										data-lstab-column="<?php echo esc_attr( (string) $lstab_i ); ?>">
										<?php echo esc_html( isset( $lstab_row[ $lstab_i ] ) ? $lstab_row[ $lstab_i ] : '' ); ?>
									</td>
								<?php endforeach; ?>
							</tr>
						<?php endforeach; ?>
			...
			<?php if ( count( $lstab_sheet_rows ) > $lstab_picker_limit ) : ?>
				<p class="lstab-help">
					<?php
					printf(
						/* translators: 1: rows shown here, 2: rows in the sheet. */
						esc_html__( 'Showing the first %1$d of %2$d rows. Hiding rows one at a time past that point is not really the tool for the job — a filter is, and Pro has one.', 'live-sheets-table' ),
						(int) $lstab_picker_limit,
						count( $lstab_sheet_rows )
					);
					?>
				</p>
			<?php endif; ?>

			<div class="lstab-picker-summary">
				<p class="lstab-picker-empty"<?php echo $lstab_hidden_rows ? ' hidden' : ''; ?>>
					<?php esc_html_e( 'No rows hidden.', 'live-sheets-table' ); ?>
				</p>
				<ul class="lstab-chips" id="lstab-hidden-rows-chips"></ul>
			</div>

			<?php /* One field per hidden row, rewritten by the picker as you click. */ ?>
			<div id="lstab-hidden-rows-fields" hidden>
				<?php foreach ( LSTAB_Hidden_Rows::sanitize( $lstab_hidden_rows ) as $lstab_hidden_key ) : ?>
					<input type="hidden" name="hidden_rows[]" value="<?php echo esc_attr( $lstab_hidden_key ); ?>">
				<?php endforeach; ?>
			</div>
		</div>
	<?php endif; ?>
